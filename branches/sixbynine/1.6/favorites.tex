...
{some\\brasilian\index{brasil}\\favorites}

\section{for those of you who...}

\begin{enumerate}
  \item[a)] no longer believe in coincidence
  \item[b)] thought this might actually have some brasilian\index{brasil} recipes
\end{enumerate}

i've tried to collect some of the more popular bahian recipes for the
\tin{Bigode} community in a desperate quest for authenticity in a globalized
culture\index{culture}. perhaps, through a few queijos quente tropicais, we'll all be
brasilian\index{brasil} one day: smiling, sexy, hungry, and \tin{100\% NEGRO}.

\subsubsection{cheese-egg}

butter both halves of an old glutinous white\index{color!white} sandwich bun or french
roll grill or toast. do not clean grill beforehand.\\
while toasting, oil nearby grill real estate and fry an egg.\\
as the egg solidifies, lay a slab of yellowish\index{color!yellow} cheese atop it.\\
...
the brasilian\index{brasil} version of yesterday's grilled-cheese sandwich, which
\tin{amanda} discovered on the menu of one of our staple internet cafes. it
had always been there i guess, as old as pineapples and slavery,
cleverly hidden amongst wicker chairs, 500 mL bowls of
\useGlosentry{acai}{a\c{c}a\'{i}} com banana, and the most amazing book i've been blessed by
browsing: \underline{\gls{frutas do brasil}}.

use three thin pieces of white\index{color!white} (or white\index{color!white}/wheat) bread,\\
...
hulls, until you see just the whiteness\index{color!white} of the beans.

\tin{robot} the soaked, rinsed, drained, hulled beans with an onion. add
...
brasil\index{brasil}) or a day in a preheated oven (in wintry temperate climes). it
...
red\index{color!red} and cooks the acaraj\'{e} a deep reddish brown.

to serve, hot!, the brasilians\index{brasil} slice down the center and add some of the following:
...
every restaurant in brasil\index{brasil} had one thing, fish, cooked in two ways:
...
\begin{algorithm}
  \item go outside and catch a red\index{color!red} fish (\gls{peixe vermelho})
...
brasil\index{brasil} (perhaps minas gerais) where the familiar black-eyed pea is
...
plenty of beer on hand to wash it down. luckily, brasilians\index{brasil} always do.
...
(UNITY\index{unity}!) and you can squeeze in some lime juice and dump in a good
