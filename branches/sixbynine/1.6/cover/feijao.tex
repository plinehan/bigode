...
mango salsa and the standard brasilian\index{brasil} table hot}
...
most simple -- no matter what your color\index{color} or size of rice is --- is
boiling.
...
short white\index{color!white} rounded grains (which you shouldn't really be using if you
...
like the organic\index{organic} red\index{color!red} rice we bought in brasil\index{brasil}, sift through it in a
...
ritual. you can add \tin{anything} you want --- cloves, soy sauce, bacon
bits, whatever.
...
later, it's already done and there's a bunch of water, EVERYTHING\index{everything} WILL
...
\subsection{the \textit{pattern} underneath \gls{arroz vermelho}}
...
\begin{itemize}
  ...
  for white\index{color!white} rice (more glutinous).
  ...
  beets or potatoes, some milk, coconut milk, tea, coffee etc. \tin{alcohol}
  ...
  a georgian wild rice earnestly simmered in a white\index{color!white} wine reduction
  ...)
  ...
  you must learn a handful of slight and clever tricks\index{trick} to save
  yourself that precious illusion\index{illusion} they call time. all this and more in
  pamphlet ``how and why to fuck the system by cooking your own food
  (and your neighbors' goddammit\index{god!damn!it})'', coming out later on in the
  kaliyuga.

\end{itemize}

\subsection{variations on red\index{color!red} rice}
...
\begin{itemize}
  ...
  \item one uses \gls{ghee} in step 2
  \item you throw in a few whole black peppercorns and whole cloves with the rice and water
\end{itemize}
...
\subsubsection{magic yellow\index{color!yellow} rice}

this simplest variation is merely to add \onehalf teaspoon of turmeric to
the water when cooking any white\index{color!white} rice. turmeric is a powerful\index{powerful} spice,
medicine, and dye, and even a tiny amount will dye the entire pot of
rice yellow\index{color!yellow}, a boon for those mindful of plating aesthetics\index{aesthetic}.

\subsubsection{magic purple\index{color!purple} rice}

another simple variation puts one diced beet into the water with the
rice. the cooking beet will turn the entire pot of rice purple{color!purple}, which
...
and stir occasionally --- the tomatoes and red\index{color!red} spices should give the
familiar orange\index{color!orange} color\index{color}.
...
\section{refried red\index{color!red} beans}
...
deep red\index{color!red} color.
...
\begin{ingredients}
  ...
  \item many cups of cooked red\index{color!red} beans (red\index{color!red} pinto looking creatures)
  \item some ground cumin (optional)
\end{ingredients}

\tin{pcook} the red\index{color!red} beans (see explanation below) and drain them. this would
of course work with any bean but the whole red\index{color!red} thing wouldn't fly as
...
now add even more dend\^{e} with the beans and stir everything\index{everything}
together. reduce the heat to medium and spend the next few moments of
your life \tin{BEING} the mixing and crushing of those beans. no bean left
...
\begin{algorithm}
  ...
  \begin{algorithm}
    ...
    \item cook together with low heat and high energy\index{energy}, mashing until
...
\subsubsection{refried \useGlosentry{fradinho}{feij\~{a}o fradinho} dip}


use the pattern above.

so for the \useGlosentry{fradinho}{feij\~{a}o fradinho} that are so ubiquitous in bahia, i
...
please for the love of{love!for the love of} martha try something else. i imagine dill- and
...
to \tin{balance} heftily on a bread or chip, and the spicing a bit too
...
\subsubsection{\useGlosentry{fradinho}{feij\~{a}o fradinho} \'{a} la \gls{mostaza}}
...
of whole black or yellow\index{color!yellow} mustard seeds. cover the lid in anticipation
...
more vigorously when the liquid \gls{vai embora}.
...
\subsubsection{\useGlosentry{fradinho}{feij\~{a}o fradinho} with fenugreek and urucum}
...
urucum. urucum, if you have it, is a red\index{color!red} seed used mainly as a
...
\begin{ingredients}
  \item 3 ripe mangos
  \item 1 good white\index{color!white} onion
...
chop everything\index{everything} together. ingredients are listed in decreasing order
...
\begin{ingredients}
  ...
  \item 1 medium white\index{color!white} onion, diced
  \item a small amount of ginger and chile pounded together
\end{ingredients}

\subsubsection{pineapple salad}

pineapple is so damn good you don't actually need \tin{anything} else. cube
...
\begin{ingredients}
  ...
  \item cheap white wine\index{color!white} to thin it out
\end{ingredients}
...
cheap during the dying hours of the public market in the \gls{cours
salaya}.

\section{standard brasilian\index{brasil} table hot}
...
what we/they have in brasil\index{brasil}; your standard supermarket
...
chop everything\index{everything} together into a small heterogeneous mixture. the
...
juices, like in brasil\index{brasil}. always cover with vinegar after using to
...
child at construction paper. \tin{anything} that tastes good by itself ---
...
the next time you're leaving the market with a measly two points\index{points} in
...
vinegar for a whimsical while. you'll have pickled \tin{XXXX} and tasty
